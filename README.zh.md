...
`./dist`。[`.github/workflows/build.yml`](.github/workflows/build.yml) 的 `site` job 负责构建并把
产物发布到 `web-build` 分支，由 Vercel / Cloudflare Pages 直接部署（宿主端无需 Flutter SDK）。
这条流水线**只能手动触发**（Actions 页 → 构建与发布 → Run workflow），所以一次 push 不会
悄悄把公开站点换掉。
