...
下文里「**服务器地址**」在排行榜与组队两节指 ej-backend（如 `http://1.2.3.4:48081`），
在 Web 展示版那节指 web-front（如 `http://<NAS>:48080`）。

> **2026-09-05 起两者合并进同一个镜像同一个容器**（仓库根的 `Dockerfile`，
> supervisord 带起两个进程，各自仍以 UID 1000 / 65532 运行、各写自己的数据目录）。
>
> **想两个一起要**：用仓库根的 `docker-compose.yml` / `docker-compose.ghcr.yml`
> 一条命令起完，端口仍是 48080 + 48081。容器内数据目录从 `/data` 变成
> `/data/backend` 与 `/data/web`，宿主挂载点不变，第一次启动会自动迁移旧布局
> （幂等、目标已存在时不覆盖）。**迁移前备份数据目录。**
>
> **只想要本文这一个**（例如 ECS 上只跑排行榜与中继）：下面这套流程仍然有效——
> `backends/docker-compose.yml` 保留着，起出来就是一个只有后端的容器。合并镜像里
> 另一个服务闲着也几乎不占资源（Rust 常驻十几 MB），两条路都行。
...
```bash
docker compose logs -f backend                        # 日志（只跑后端那条路）
...
用合并镜像时两个服务的日志都在 `ej-app` 一个容器里（`docker compose logs -f app`），
也可以只重启其中一个而不动另一个：

```bash
docker exec ej-app supervisorctl -c /etc/supervisor/conf.d/supervisord.conf restart backend
```
...
连不上时：**组队设置 → 「测试连接」**先跑一遍（它会检查服务可达、服务端的组队模块是否
启用、WebSocket 能否升级）；仍不清楚再看 **诊断日志**。
常见原因：地址少了 `http(s)://`、令牌不一致、两端口令不一致（能连上但看不到消息，
日志会提示「解密失败」——这一项测试**测不出来**，只能从日志看）。

> 令牌不对时 WebSocket 升级那一步**会失败，但报不出「是令牌的问题」**：服务端回的是
> 一个裸的 `HTTP/1.1 401`，而 Dart 的 WebSocket 客户端在升级被拒时不把状态码交出来。
> 所以那一步只会告诉你「被拒绝」，并把**令牌不一致列为首要嫌疑**、反代没转发
> `Upgrade` 头列为次要。要确认是哪一个，对比 App 里的中继令牌与服务端 compose 里的
> `GROUP_TOKEN` 是最快的一步。
