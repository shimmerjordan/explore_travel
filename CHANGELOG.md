...
- **年度 / 旅程总结卡**：把一段时间的足迹渲成一张 1080×1920 的可分享竖图。一套实现两个入口——足迹统计页右上角出「今年」的卡，回放页（单段或合并记录）出「这段旅程」的卡。卡上是里程、有记录的天数、最长连续、国家数、24 格作息带、待得最久的几处地点，以及走出来的形状。

  形状取自**该范围内的轨迹点**而不是迷雾位图：`fog_tiles` 只有块级 `updatedAt`，取不出「某年点亮的那部分」（与之前否掉「时间机器」是同一个数据模型限制）。整段历史都由 FOW 位图导入的用户没有轨迹点，那时卡片明说「没有轨迹可画」，不摆一张空画布。反查不到名字的地点（「未命名地点」）不上卡，判据与时间轴页一致。

  抓图走屏上的 `RepaintBoundary`（与回放导出视频同一套，已真机验证），先给用户看预览再决定分不分享。

### 修复（都是这次加按钮时暴露出来的）

加按钮拿掉了一个此前一直成立的隐含前提：**用户要按到停止键，必然先把应用切回前台**。四个缺陷都由此而来，经一轮多视角对抗审查（4 个视角提指控、每条 2 名独立裁决者证伪）找出：

- **轨迹丢失**：`stop()` 直接截断样本缓冲文件。以前界面按钮那条路上，切回前台已触发过一轮排空；通知按钮没有这个前提，于是**只存在于缓冲里**的样本（队列闸门在 Doze 成串到达时丢弃的、写库失败的）被连文件一起删掉。现在改为先排空再清文件。变异测试为证：去掉修复，回归测试立刻失败。
- **假录制态**：停止命令若落在 `start()` / `resumeIfRecording()` 的接线窗口（权限申请 + 启服 + 排空，实测数秒），旧守卫按 `recordingActive` 判断会把请求静默吞掉，接线随后还把状态置成「正在记录」——用户卡在「显示在录、服务已停」。改为记录停止意图，接线尾部据此转去收尾。
- **收尾丢失**：主 isolate 不在场时命令随不存在的端口被丢弃，那段会话的缓冲排空与到访检测永远不跑，冷启动也不补。现在冷启动时若缓冲非空会补做一次。
- `stop()` 中间任一平台通道抛异常会让幂等闸门永久卡住，用户从此再也停不下来。改用 `try/finally`。

顺带修掉两个既有缺陷：`_bgSub` 被覆盖而不 cancel（插件里的回调从此永不注销）、`start()` 没有重入闸门。

真机验证（M2012K11C，全程未把应用切到前台）：通知按钮存在且可点，点击后服务停止、定位请求归零、主 isolate 完成收尾、缓冲排空且轨迹点无丢失。
