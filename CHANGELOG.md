...
- **组队设置页新增「测试连接」**：按当前选中的通道跑一遍到**协议握手层**的探测，逐条
  给结果与失败提示。不是「能 ping 通」——中继要真的完成 WebSocket 升级（令牌错在这步
  暴露）、WebDAV 要真的写进去再读回来、frp 要真的让 frpc 登录并注册上 xtcp proxy、
  局域网要真的能绑 mesh 端口并发出多播。
- **两处刻意的诚实**：局域网发现 0 个对端**不算失败**（对端不在线不是本机配置问题），
  本机就绪与对端数分开报；「共享口令与对方是否一致」标记为**无法验证**并说明原因
  （单机测不出来，症状是能连上但看不到消息）——一个全绿的报告不该给人错误的信心。
- frp 通道在组队**正在运行**时读现有 frpc 状态而不重启，「用着的时候查为什么连不上」
  不需要先断开。
